...
type MoveCmptProp = {
  step: number;
  elMoveChange?: (changeVal: AliasFn, checkRun: CheckRunFn) => void
}
...
function MoveComponent(props: MoveCmptProp) {
  ...
  const autoelMoveChange = () => {
    props.elMoveChange?.(setPosition, checkRun);
  }
  ...
  const handleClickDown = (event: MouseEvent) => {

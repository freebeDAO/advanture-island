...
const MovableComponent = forwardRef((props: MovableProps, moveOutRef) => {
    const { children, parentRef, width, height, defaultSpeed } = {
        ...defaultProps,
        ...props,
    };
    ...
    const getXY = () => {
        return {
            x: moveRef.current.style.left
                ? parseInt(moveRef.current.style.left.replace("px", ""))
                : 0,
            y: moveRef.current.style.top
                ? parseInt(moveRef.current.style.top.replace("px", ""))
                : 0,
        };
    };
    ...
    function limitTargetPoint(
        dX: number,
        dY: number,
        newX: number,
        newY: number,
        target: { newLeft: number; newTop: number }
    ) {
        let { minX, minY, maxX, maxY } = {
            minX: 0,
            minY: 0,
    ...
    const move = (
        dX: number,
        dY: number,
        target: { newLeft: number; newTop: number } | null,
        isAuto?: boolean
    ) => {
        ...
        let { ratioX, ratioY } = { ratioX: 1.0, ratioY: 1.0 };
        ...
        if (isAuto) {
            ...
            if (
                dX == 1 &&
                newX ==
                    parentRef.current.clientWidth - moveRef.current.clientWidth
            ) {
                isOver = true;
            }
            if (dY == -1 && newY == 0) {
            ...
            if (
                ...
                newY ==
                    parentRef.current.clientHeight -
                        moveRef.current.clientHeight
            ) {
                isOver = true;
            }
            if (isOver) {
                // console.log("autoMove over");
                return true;
            }
            // if (
            //     (newX == 0 && newY == 0) ||
        ...
        if (speed > 0) {
            let poitId = setInterval(
                () => {
                    ...
                    // console.log(`${isOver}`);
                    if (isOver) {
                        // console.log(`timeId:${poitId}`);
                        clearInterval(poitId);
                        // console.log(`clearInterval point:${poitId}`);
                        setTimerId(null);
                        // setIsMovable(false);
                        isMoving = false;
                    }
            ...
            let autoId = setInterval(
                () => {
                    ...
                    if (isOver) {
                        ...
                        // console.log(`clearInterval auto:${autoId}`);
                        setTimerId(null);
                        isMoving = false;
                    }
                },
    ...
    function handleMouseClick(e: MouseEvent) {
        // console.log(`handleMouseClick isMoving:${isMoving}`);
        if (!isMoving) {
